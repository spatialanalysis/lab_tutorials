...
This notebook cover the functionality of the [Spatial Weights as Distance Functions](https://geodacenter.github.io/workbook/4c_distance_functions/lab4c.html) section of the GeoDa workbook. We refer to that document for details on the methodology, references, etc. The goal of these notes is to approximate as closely as possible the operations carried out using GeoDa by means of a range of R packages.
...
function, with a value of 1 for neighbors with $d_{ij} < δ$, and a value of 0 for others. As before, 
$d_{ij}$ stands for the distance between observations i and j, and δ is the bandwidth.
...
to distance should be negative, $∂wij/∂dij<0$
.

Commonly used distance functions are the inverse, with $w_{ij}=1/d_{ij}^α$(and α as a parameter), and the negative exponential, with $w_{ij}=e^{−βd_{ij}}$(and β as a parameter). The functions are often 
combined with a distance cut-off criterion, such that $w_{ij}=0$ for d_{ij}>δ.

In practice, the parameters are seldom estimated, but typically set to a fixed value, such as 
α=1 for inverse distance weights ($1/d_{ij}$), and α=2 for gravity weights ($1/d_{ij}^2$). By convention, 
...
distances between each neighbors stored in the same format as the neighbors. Then
...
Here we begin by putting our coordinates in a separate matrix from **clev.points**
```{r}
...
correspond in order to assign the inverse disatnces as the weight values when converting from
a neighbors list or class **nb** to a weight structure: class **listw**.
```{r}
invd1[1]
```
A key insight from the first element of the inverse distance structure is that the values 
very small, or too close to zero. The units of distance for our dataset is in feet. This
means distance values between points can be quite large and result in small inverses. To 
correst for this scale dependence, we can rescale the distances by repeating the inverse
calculates, while adjusting the scale. We can make this adjustment by dividing **x** in
...
Now that we have properly scaled inverse disatnces, we can assign them as weight values. 
...
Here we take a cursory look at our weights with `summary` to basic imformation and statistics.
...
Adding these points to the neighbors list is a bit tricky, as we have to add them to each
row in the neighbors list and they need to go in particular spot in the respective row
to avoid errors later on. Essentially what we need to do is loop through each row in
the neighbors list, append the corresponding diagonal element, then sort the row.

To do this, we use a `for` loop with i-values 1 to the length of the neighbors list. We 
can access a row of the neighbors list through the variable name followed by double 
brackets with the corresponding index. Once we have accessed a row, we use the `append`
function to add the matching diagonal element, which will be **i**. Then we use the 
`sort` function to put the row in increasing order, like all our neighbors lists have
been up to this points.

```{r}
for (i in 1:length(nb.dist.band)){
  new_row <- sort(append(kernal.nb[[i]], i, after = 0), decreasing = FALSE)
  kernal.nb[[i]] <- new_row
}
```
