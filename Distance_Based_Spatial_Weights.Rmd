...
This notebook cover the functionality of the [Distance-Band Spatial Weights](https://geodacenter.github.io/workbook/4b_dist_weights/lab4b.html) section of the GeoDa workbook. We refer to that document for details on the methodology, references, etc. The goal of these notes is to approximate as closely as possible the operations carried out using GeoDa by means of a range of R packages.
...
- Understand the use of great cirlce distance
...
To get a cursory look at the pojnt data with a basemap, we will use **tmap**. To get 
this done we first ahve to switch from mode `plot` to mode `view` with the function
...
The most straightforward spatial weights matrix constructed from a distance measure is obtained when i and j are considered neighbors whenever j falls within a critical distance band from i. More precisely, $w_{ij}=1$ when $d_{ij} \leq \delta $ , and $w_{ij}=0$ otherwise, where δ is a preset critical distance cutoff.
...
will give us a list, where each point has exactly one neighbor. To do this, we first
use the 'knearneigh` function from **spdep** library. This will give us a class of
**knn**, which is similar to class **nb**, but we will need to convert anyways.
...
neighbors. To make this neighbors list we use `dnearneigh` from the **spdep** package.
The parameters necessary are the coordinates, the lower distance bound, and the upper
distance bound. We enter these in the the above order. Another impoartant paramter is
the `longlat`. This is used for point data in longitude and latitude form. It is
necessary to use this to great circle distance instead of euclidean, for accuracy
purposes.
...
weights object. This will give us alot of useful information, that we cannot get with
the `str` command.
```{r}
...
of neighbors for each observation.
...
we use `geom_histogram` to add our histogram. The only paramter we need is an aes.
...
dialog is flexible enough that we can type in any value for the cut-off, or use the
moveable button to drag to any value larger than the minimum. Sometimes, theoretical
...
actually closer to B than A. This asymmetry can cause problems in analyses that depend
on the instrinsic symmetry of the weights (e.g., some algorithms to estimate spatial
...
convert to a data form we can use for the contiguity weights. We won't go into
...
command to visualize these because here is a built in method for **deldir** thiessen polygons. To get a good visual of our polygons, we need a few extra paramters.
...
Using the **sf** function `st_as_sf`, we can convert to **sf** from our SpatialPolygonsDaraFrame.
...
With our conversion fucntion, we can covert to class **nb**.
...
We can take a look at the distrubtion of the number of neighbors in our queen contiguity
...
for polygons. To illustrate this, we will use U.S homocide data from a previous
notebook(the contiguity-based weights notebook).

To implement this we will start by computing shape center coordinates for the
...
Getting our coordinate data is a little more complicate dthan just using `st_centroid`
on our shapefile. We need to do this to get coordinates in a supported format for
the neighbor functions in the *spdep** package. We are going to get a vector for
latitude and one for longitude, then will bind the columns for later use.

To get our centroid coordinates, we will map the function `st_centroid` over the
geometry column of **us.bound**. The mapping functionapplies a given function to each 
element of a vector and returns a vector of the same length. Our input vector will be 
the geometry column of **us.bound**. Our function will be `st_centroid`. We will be using `map_dbl` variation of `map` from the **purrr** package. For more documentation, check out [map documentation](https://www.rdocumentation.org/packages/purrr/versions/0.2.5/topics/map)

To get longitude values, we need double bracket notation  and 1 to access the first
value.
...
Now we bind the coordinates together with `cbind`, so we can enter them as paramters
...
To get the critical threshold, we folllow the same steps as earlier, with one difference.
...
Once we have caluculated the distance-band weights, we can assess the distribution of the number of 
