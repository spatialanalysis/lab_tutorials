...
For example, the default theme with the grey grids, separated by white lines is
...
information using the `layer_data` function.
...
between them by setting the `color` argument in `aes` to the method.
In other words, the color argument is not set to a variable, but to a constant.
...
argument used to reflect the categories. We override this by means of the `labs` command, and set `color="Method"`. The result is as shown below.
...
Finally, we implement the Chow test as `chow.test`. We pass the y and x variables for each subset, in turn. Again, the results are the same as what one would have obtained in GeoDa.
