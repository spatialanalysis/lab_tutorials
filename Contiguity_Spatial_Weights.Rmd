...
To get the data for this notebook, you will and to go to [US Homocides](https://geodacenter.github.io/data-and-lab/ncovr/) The download format is a
...
```{r}
us.bound <- st_read("ncovr/NAT.shp")
```
...
We will create our neighbours using **sf** first, as the **spdep** library doesn't 
allow us to create neighbours lists directy with as with the older **sp** library. 
When we create neighbours in **sf** we will get a class of **sgbp**(sparse geometry binary predicate), which is similar
...
We first start with rook contiguity, whic will be neighbours that are connected by a
common side. To create our neighbours list we first start by making a 
...
or match it to a given pattern. This function also has a parameter for a specified 
...
We now check the class of our resulting object from the the `st_intersect` function
with the base R function `class`. It is **sgbp**, which we will have to change to
...
the `str` command. We will se a list of 3085 lists that contain the id numbers of
each observation's neighbours. Along with this we will see some attributes. These
...
observations with no neighbours with the integer 0. Our data set doesn't have
any observations without neighbours, but in ones with these, it will mess everything
...
need to reapply them later. Now we deal we observation with no neighbours. We will
...
0(meaning no neighbours) and returns 0 if the element is empty and the element
...
```{r}
#alternative method for getting queen contiguity neighbors in sf
...
The `nblag` function takes a neighbour list of class **nb** and an order as parameters.
It will give us a list of neighbours lists. One for 1st order neighbors and one for
second order neighbours. We can select from this data structure by double bracket
notation, for instance `[[1]]` will give the **nb** object for first order neighbours.
Using a `2` instead of a 1 will give us the second order neighbours
...
We will first take a comprehensive look at **second.order.queen** to see the resulting
data structure from `nblag`. As said earlier, it is a list of 2 **nb** objects. One
for 1st order neighbours and one for 2nd order. It is important to examine this, so
...
Now we can look at, specifically, the 2nd order neighbours by using the double bracket
selection.
...
Now if we want to include both 1st and 2nd order neighbours in our visualizations, we
need the `nblag_cumul` function. This will give us an **nb** object with both 1st and 2nd order neighbours, instead of separating them as in the result of `nblag`. To use this
function, we need a resulting object from the `nblag` function. It conglomerates the two sepatae **nb** objects from the `nblag` function into one.
...
A connectivity graph takes a point and displays line to each neighboring point. We are
...
#### Getting Latitude and Longiitud of Polygon Centroids
...
for this to work. To do this we will us a mapping function. The mapping function
...
Now that we have coordinates for each of our observations, we can plot our neighbours
...
add further customizations, so we can actually see what is going on in the graph.
...
as there are more neighbours due to shared vertices counting in the neighbours
relationship. It is hard to spot the differences between the two plots, as they
both are visually busy 
...
Here we take a look at a connectivity graph for second order neighbours. All we need
is the second order **nb** object that we created earlier with our coordinates. This
graph will look significantly different from the ones we've made so far, as second 
order tends to have more neighbours than first order.
...
We will take a look a plot with both second and first order neighbours here. It will
...
A connectivity histogram shows the number of observations for each value of the
cardinality of neighbours(how many observations have the same number of neighbours).
...
number of neighbours from the input neighbours list.
...
it is a vector with number of neighbours for each observation of the **nb** object.
```{r}
...
the most sense as you cannot have half of a neighbour.
...
statistics for the cardinality of the neighbours list.
```{r}
summary(rook.card)
```
...
## Saving Neighbors
To save our neighbours list, we use the `write.nb.gal` function from
the **spdep** package. The file format is a GAL Lattice file. We
input our neighbours list, and the the filename second. We have two 
options from this point. We can save the file with the old style
or the new GeoDa header style.

### Oldstyle

The oldstyle just saves the neighbour list with then number of observations
