...
Again, the layer is unprojected in decimal degrees. Also, a quick `plot`.
...
The **pop.dat** object has two entries, one for each page. Each entry is a string. So, when you check the length of each item, it may be surprising that its **length** is only 1. That is because the underlying structure is unknown, it is simply a collection of characters. For example, the first element, **pop.dat[[1]]**:
...
The resulting list, **pop.dat[[1]]**, contains a list of 44 elements, matching the contents of the first page of the pdf file.
...
Each element is one long string, corresponding to a table row. We remove the first four lines (using the - operation on the list elements 1 through 4).
