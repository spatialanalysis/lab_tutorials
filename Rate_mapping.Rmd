...
- **tmap**: `tm_shape`, `tm_fill`, `tm_border`, `tm_layout`, `tmap_mode`, 
...
The flip side of this result is that for areas with sparse populations (small $P_i$), the estimate for the risk will be imprecise (large variance).
...
first as opposed to GeoDa which does it automatically. We start by calculating

```{r}
sum_observed <- sum(ohio_lung$LFW68)
sum_population <- sum(ohio_lung$POPFW68)
pi <- sum_observed / sum_population
```


```{r}
E_i <- pi * ohio_lung$POPFW68
...
for(i in nrow(ohio_lung)){
  
}

nb_matrix <- diag(length(ohio_lung$LFW68))

weight_matrix <- mat2listw(nb_matrix)

eb_bayes <- EBlocal(O_i, P_i, nb_matrix)


mu <- sum(ohio_lung$LFW68) / sum(ohio_lung$POPFW68)
O_i <- ohio_lung$POPFW68
P_i <- ohio_lung$LFW68
n <- length(ohio_lung$POPFW68)
r_i <- P_i / O_i
top_left <- sum(P_i * (r_i - mu)^2)
variance <- top_left / sum(P_i) - mu / sum(P_i / n)
variance <- (variance - abs(variance)/2)
variance <- 0
